    ...
    public function login($username, $pass, $remember = FALSE) {
        ...
        if ($query->num_rows() > 0) {
			...
			foreach ($query2->result_array() as $key => $row3 )
			{
			   $menu[] = array(
			   		'menu_key' => $key,
			   		'menu_matrix_id' => $row3['user_matrix_id'],
			   		'menu_modul_id' => $row3['modul_id'],
			   		'menu_modul_name' => $row3['modul'],
			   		'menu_cipta' => $row3['cipta'],
			   		'menu_baca' => $row3['baca'],
			   		'menu_kemaskini' => $row3['kemaskini'],
			   		'menu_hapus' => $row3['hapus'],
			   		'menu_sah' => $row3['sah'],
			   		'menu_lulus' => $row3['lulus']
				);
			}	
			//print_r($menu);
