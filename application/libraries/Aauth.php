...
//////////////////////////	DISABLE SAT NO ///////////////////////////
			...
			//if($query2->num_rows > 1){echo "<script>alert('role lebih 1')</ script>";}
			//else{echo "<script>alert('role 1 jer!')</ script>";}
...
*/
            ...
            $data = array(
				...
				'user_level_id' => $row->level_id,
				'user_level' => $this->util_get_level($row->level_id),
			...
			///////// CEK DULU WEI ADA 1 KE 2 ROLE //////////
/*
			$cekrole = $this->check_role($row->myspata_userid);
			if($cekrole->num_rows > 1){
				//echo "lebih 1";
				redirect('auth/pilih_role/'.$row->myspata_userid,'refresh');
			}
*/			
			///////// CEK DULU WEI DAH ABIS SINI ////////////
	...
	public function check_role($id){
		...
		$this->CI->db->select('*');
		$this->CI->db->from('tbl_myspata_user');
		$this->CI->db->join('tbl_myspata_user_to_matrix','tbl_myspata_user.myspata_userid = tbl_myspata_user_to_matrix.myspata_userid');
		$this->CI->db->where('tbl_myspata_user.myspata_userid',$id);
    ...
    public function util_get_level($id) {

        $query = $this->CI->db->where('level_id', $id);
        $query = $this->CI->db->get('lkp_level');

        if ($query->num_rows() == 0)
            return FALSE;

        $row = $query->row();
        return $row->level_desc;
    }
