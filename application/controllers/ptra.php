...
function status_premis_ptra()
        ...
        if($_POST)
            {
                        ...
                        $this->form_validation->set_rules('kumpagensi','Kumpulan Agensi','trim|required|xss_clean');
                        ...
                        $this->form_validation->set_rules('mukim','Mukim','trim|required|xss_clean');
                        $this->form_validation->set_rules('kat_premis','Kategori Premis Aset','trim|required|xss_clean');
  ...
  function kat_jalan_ptra()
  {
    ...
    if($getPnpa1 = $this->function_model->get_tab(2))
        ...
        $data['jabatan'] = $this->ptra_model->get_jabatan_agen(); //dapatkan senarai jabatan dr db
        $data['negara'] = $this->ptra_model->get_negara(); //dapatkan senarai negara dr db
        $data['katagensi'] = $this->ptra_model->get_katagensi();
        ...
        if($_POST)
            ...
            if($this->form_validation->run())
                         {
                    $ptra_reg=$this->ptra_model->tambahprajlnptra();
                       //return $ptra_reg;
                        //echo $ptra_reg;
                         redirect('ptra/kandungan_ptra/'.$this->uri->segment(3).'/0/'.$ptra_reg,'refresh'); 
              //redirect('ptra/kandungan_ptra/'.$this->uri->segment(3),'refresh');
  ...
  function kat_pembetungan_ptra()
  {
    ...
    if($getPnpa1 = $this->function_model->get_tab(2))
        ...
        if($_POST)
            ...
            if($this->form_validation->run())
                         {
                            $ptra_reg=$this->ptra_model->tambahprapemptra();
                       //return $ptra_reg;
                        //echo $ptra_reg;
                             redirect('ptra/kandungan_ptra/'.$this->uri->segment(3).'/0/'.$ptra_reg,'refresh'); 
              //redirect('ptra/kandungan_ptra/'.$this->uri->segment(3),'refresh');
  ...
  function kat_saliran_ptra()
  {
   ...
   if($getPnpa1 = $this->function_model->get_tab(2))
        ...
        if($_POST)
            ...
            if($this->form_validation->run())
                         {
                            $ptra_reg=$this->ptra_model->tambahprasaliranptra();
                       //return $ptra_reg;
                        //echo $ptra_reg;
                             redirect('ptra/kandungan_ptra/'.$this->uri->segment(3).'/0/'.$ptra_reg,'refresh'); 
              //redirect('ptra/kandungan_ptra/'.$this->uri->segment(3),'refresh');
  ...
  function pra_reg_edit()
  {
    ...
    $node_id = '14';
    $menu_name = 'menu1';
    $menu_link = 'ptra/pra_reg_edit';
                
    $data = array('menu_name' => $menu_name, 'menu_id' => $node_id, 'main_content' => $menu_link);
    $data['menu_parent'] = $this->sidemenu_model->get_sidemenu_parent($menu_name);
    $data['menu_sub'] = $this->sidemenu_model->get_sidemenu_sub($menu_name);
    $data['menu'] = $this->sidemenu_model->get_sidemenu($menu_name);
    $idpremisreg= $this->uri->segment(5); 
    if($getPnpa1 = $this->function_model->get_tab(2))
                    {
                       $data['tab'] = $getPnpa1;
                    }
        ...
        if($_POST)
            {
                 //echo $idpremisreg; 
                // die();
             if($this->input->post('simpanbangunan') == 'Simpan')
                 {
                        ...
                        if($this->form_validation->run())
                         {
                                ...
                                $nodpa=$this->ptra_model->updatepraptra($idpremisreg);
                                 $this->ptra_model->updateptradpa($nodpa);
             ...
             else if($this->input->post('simpanjalan') == 'Simpan')
                 {
           
                        $this->form_validation->set_rules('kumpagensijalan','Kumpulan Agensi','trim|required|xss_clean');
                        $this->form_validation->set_rules('jabatanjalan','Jabatan','trim|required|xss_clean');
                        $this->form_validation->set_rules('negarajalan','Negara','trim|required|xss_clean');
                       $this->form_validation->set_rules('kat_premisjalan','Kategori Premis Aset','trim|required|xss_clean');
                       
                        if($this->form_validation->run())
                         {
                            
                                $nodpa=$this->ptra_model->updatepraptrajalan($idpremisreg);
                                 $this->ptra_model->updateptradpa($nodpa);
             ...
             else if($this->input->post('simpanpem') == 'Simpan')
                 {
           
                $this->form_validation->set_rules('kumpagensijalan','Kumpulan Agensi','trim|required|xss_clean');
                $this->form_validation->set_rules('jabatanjalan','Jabatan','trim|required|xss_clean');
                $this->form_validation->set_rules('negarajalan','Negara','trim|required|xss_clean');
                $this->form_validation->set_rules('negerijalan','Negeri','trim|required|xss_clean');
                $this->form_validation->set_rules('daerahjalan','Daerah','trim|required|xss_clean');
                $this->form_validation->set_rules('kat_premisjalan','Kategori Premis Aset','trim|required|xss_clean');
                    
                        if($this->form_validation->run())
                         {
                            
                                $nodpa=$this->ptra_model->updatepraptrapem($idpremisreg);
                                 $this->ptra_model->updateptradpa($nodpa);
             ...
             else if($this->input->post('simpansaliran') == 'Simpan')
                 {
                 $this->form_validation->set_rules('kumpagensisaliran','Kumpulan Agensi','trim|required|xss_clean');
                $this->form_validation->set_rules('jabatansaliran','Jabatan','trim|required|xss_clean');
                $this->form_validation->set_rules('negarasaliran','Negara','trim|required|xss_clean');
                $this->form_validation->set_rules('kat_premissaliran','Kategori Premis Aset','trim|required|xss_clean');
                  
                        if($this->form_validation->run())
                         {
                            
                                $nodpa=$this->ptra_model->updatepraptrasaliran($idpremisreg);
                                 $this->ptra_model->updateptradpa($nodpa);
...
////here start copy from PTRA////  
   ...
   function kandungan_ptra()
  {
    ...
    //$data['main_content'] = 'ptra/kandungan_ptra';
        ...
        $dataYear = $this->ptra_model->get_year_pspao($this->uri->segment(3));
							...
							if($this->upload->do_upload('userfile'))
								{
									...
									$this->utilitikeperluansumber_model->tambahsumberpanels($dataUpload['full_path']);
...
function set_skop_aktiviti_ptra($idsumber)
  ...
  //edited : fatin 13/11/13
        ...
        if($_POST)
        {
                ...
                if($this->form_validation->run())
                {
                //$data['ptra_pata_f8_1d_id'] = $this->session->userdata('ptra_pata_f8_1d_id');
                $addRekod = $this->ptra_model->tambahkawalan_rekod();
                
               // $ptra_pata_f8_1d_id = $this->input->post('ptra_pata_f8_1d_id');                
                //$this->session->set_userdata(array('ptra_pata_f8_1d_id' => $ptra_pata_f8_1d_id));
...
function kemaskinikawalan_rekod_ptf()
  ...
  */
         ...
         function summary_ptra ()
  {
                ...
                if($getstatus = $this->function_model->check_status_log($id,2,$ptra_id))
                 {
                ...
                //$data['statusid'] = $this->function_model->check_status_log($id,4,$ptra_id);
                 if($getulasan = $this->ptra_model->get_ulasan_terbaru($id,2,$ptra_id))
                 {
        ...
        function summary_pp_ptra ()
  {
                ...
                $ptra_id = $this->uri->segment(4);
                $data['rows'] = $this->ptra_model->get_all_kandungan_ptra($ptra_id);
                $data['statusid'] = $this->function_model->check_status_log($id,2,$ptra_id);
                 $data['ulasan'] = $this->ptra_model->get_ulasan_terbaru($id,2,$ptra_id);
                if($getPtra = $this->ptra_model->get_ptra2($id,$ptra_id))
                {
                   $data['senarai_ptra'] = $getPtra;
                }
...
if($this->input->post('sah') == 'sah')
                    {
                          ...
                          $this->ptra_model->update_ptra_date_sah($id,$ptra_id); //update sah date

                          $this->ptra_model->insert_status_log_ulasan($id,4,$ptra_id); //insert status & ulasan

                    /****** hantar notifikasi kpd pp ada ptra perlu diluluskan ******/
                           $path = 'ptra/summary_pp_ptra/'.$id.'/'.$ptra_id;
                           $this->function_model->insert_notifikasi(107,2,$sessionarray['user_id'],$this->ptra_model->get_ppid_ptra($id),$path); //insert notifikasi

                    /****** hantar notifikasi kpd ppd ada ptra telah disahkan******/
                           $path2 = 'ptra/summary_ptra/'.$id.'/'.$ptra_id;
                           $this->function_model->insert_notifikasi('54',2,$sessionarray['user_id'],$this->ptra_model->get_ppdid_ptra($id,$ptra_id),$path2); //insert notifikasi

                            //$data['detail_msg'] = $this->function_model->get_masej($id,7);
                            $data['main_content'] = 'alert';
                            $data['msg'] = 'Borang PTRA  telah berjaya dihantar untuk diluluskan. Klik butang OK untuk kembali ke Senarai PTRA.';
                            $data['link'] = 'ptra/senarai_ppd_ptra/'.$id;
                ...
                //redirect('pspao/senarai_pspao_akhir','refresh');
                        ...
                        $this->ptra_model->insert_status_log_ulasan($id,3,$ptra_id); //insert status & ulasan
                         ...
                         /****** hantar notifikasi kpd ppd ada pspao perlu dibetulkan ******/
                          ...
                          //$pspao_awal_ppd_id   = $this->pspao_model->get_pspao_ppd_id($pspaid); // get ppd id
                           ...
                           $path3 = 'ptra/summary_ptra/'.$id.'/'.$ptra_id;

                           $this->function_model->insert_notifikasi(55,2,$sessionarray['user_id'],$this->ptra_model->get_ppdid_ptra($id,$ptra_id),$path3); //insert notifikasi
...
//added :diana 20/11/13
	function muat_suratlantikan()
  {
    ...
    $lokasiFile = $this->ptra_model->get_suratInfo($lampiranID);
	...
	//added :diana 5/12/13
  function get_daerahnegeri($idnegeri)
	{
		$this->load->model('ptra_model');
		header('Content-Type: application/x-json; charset=utf-8');
		echo(json_encode($this->ptra_model->get_daerah_negeri($idnegeri)));
	}
	
	//added :diana 5/12/13
  function get_mukimdaerah($idnegeri,$iddaerah)
	{
		$this->load->model('ptra_model');
		header('Content-Type: application/x-json; charset=utf-8');
		echo(json_encode($this->ptra_model->get_mukim_daerahnegeri($idnegeri,$iddaerah)));
	}
	
	function status_premis_ptra_copy()
	{
		//added : diana 6/12/13

		$node_id = '14';
		$menu_name = 'menu1';
		$menu_link = 'ptra/status_premis_ptra_copy';
        ...
        {
             if($this->input->post('status')=='baru' && $this->input->post('kataset')=='bangunan')
                {  redirect('ptra/kat_bangunan_ptra_copy/'.$this->uri->segment(3).'/'.$this->uri->segment(4));
                
                }
            else if($this->input->post('status')=='baru' && $this->input->post('kataset')=='jalan')
                {  redirect('ptra/kat_jalan_ptra_copy/'.$this->uri->segment(3).'/'.$this->uri->segment(4));
                
                }
            else if($this->input->post('status')=='baru' && $this->input->post('kataset')=='pembetungan')
                {  redirect('ptra/kat_pembetungan_ptra_copy/'.$this->uri->segment(3).'/'.$this->uri->segment(4));
                
                }
			else if($this->input->post('status')=='baru' && $this->input->post('kataset')=='saliran')
                {  redirect('ptra/kat_saliran_ptra_copy/'.$this->uri->segment(3).'/'.$this->uri->segment(4));
                
                }
			else if($this->input->post('status')=='sedia')
                {  redirect('ptra/kandungan_ptra_copy/'.$this->uri->segment(3).'/'.$this->uri->segment(4));
	...
	function kat_bangunan_ptra_copy()
	{
		$node_id = '14';
		$menu_name = 'menu1';
		$menu_link = 'ptra/kat_bangunan_ptra_copy';
		...
		if($getPtra = $this->function_model->get_tab(2))
		{
		   $data['tab'] = $getPtra;
		}
        ...
        if($_POST)
        {
            ...
            if($this->form_validation->run())
            {
				$ptra_reg = $this->ptra_model->tambahpraptra_copy($this->uri->segment(4));
                
                redirect('ptra/kandungan_ptra_copy/'.$this->uri->segment(3).'/'.$this->uri->segment(4),'refresh'); 
  ...
  function kat_jalan_ptra_copy()
  {
		//added : diana 6/12/13
		$node_id = '14';
		$menu_name = 'menu1';
		$menu_link = 'ptra/kat_jalan_ptra_copy';
        ...
        if($_POST)
        {
				...
				if($this->form_validation->run())
                {
                    $ptra_reg=$this->ptra_model->tambahprajlnptra_copy($this->uri->segment(4));
                    redirect('ptra/kandungan_ptra/'.$this->uri->segment(3).'/'.$this->uri->segment(4),'refresh'); 
				}
                else
                {
					$this->load->view('template/default', $data);
  ...
  function kat_pembetungan_ptra_copy()
  {
    
	//added : diana 6/12/13
    $node_id = '14';
    $menu_name = 'menu1';
    $menu_link = 'ptra/kat_pembetungan_ptra_copy';
	...
	$data['katagensi'] = $this->ptra_model->get_katagensi();
			...
			if($_POST)
            {
				...
				if($this->form_validation->run())
				{
					$ptra_reg=$this->ptra_model->tambahprapemptra_copy($this->uri->segment(4));
					redirect('ptra/kandungan_ptra/'.$this->uri->segment(3).'/'.$this->uri->segment(4),'refresh'); 
				}
                else
                {
  ...
  function kat_saliran_ptra_copy()
  {
    //added : diana 6/12/13

    $node_id = '14';
    $menu_name = 'menu1';
    $menu_link = 'ptra/kat_saliran_ptra_copy';
        ...
        if($_POST)
            {
				...
				if($this->form_validation->run())
                {
                    $ptra_reg=$this->ptra_model->tambahprasaliranptra_copy($this->uri->segment(4));
                    redirect('ptra/kandungan_ptra_copy/'.$this->uri->segment(3).'/'.$this->uri->segment(4),'refresh'); 
				}
                else
                {
  ...
  //added : diana 8/12/13
   function kandungan_ptra_copy()
  {
        $node_id = '14';
		$menu_name = 'menu1';
		$menu_link = 'ptra/kandungan_ptra_copy';
        ...
        if($_POST)
       {
			...
			if($this->form_validation->run())
			{
			  ...
			  if($this->input->post('edit')!= NULL)
			  {
				...
				$this->salinplan_model->updateborang_copy('ptra');
   ...
   //added : diana 13/11/13 : function copy borang
	function copyborang()
	{		
		$ptraIDCopy = $this->uri->segment(4);
		//$typeofPlan = $this->uri->segment(5); //1-pnpa,2-ptra
		
		//if($typeofPlan==1)
		//{
			if($this->salinplan_model->checkrekodcopy($ptraIDCopy,'ptra')==1)
			{
				$chk = $this->salinplan_model->set_copy_rekod($ptraIDCopy,'ptra');
				
				if($chk)
				{
					redirect('ptra/senarai_ppd_ptra/'.$this->uri->segment(3),'refresh');
				} 
				else
				{
					$this->session->set_flashdata('flashError', '<strong><font color="red" size="3">AMARAN : Gagal menyimpan rekod.</font><br></strong><br>');
			...
			{
				$this->session->set_flashdata('flashError', '<strong><font color="red" size="3">AMARAN : Gagal menyimpan rekod.</font><br></strong><br>');
				redirect('ptra/senarai_ppd_ptra/'.$this->uri->segment(3).'/'.$this->uri->segment(4),'refresh');
			}
		//}
	}
