...
class Pentadbir extends CI_Controller {
  ...
  function reset_pengguna()
  {
    ...
    if($getListuser = $this->pentadbir_model->get_listuser_1($myspata_userid))
    {
    ...
    //$this->form_validation->set_rules('myspata_userid','User ID','trim|required|xss_clean');
    $this->form_validation->set_rules('password','Kata Laluan Baru','trim|required|xss_clean');
    //$this->form_validation->set_rules('nokp','No. Kad Pengenalan','trim|required|xss_clean');
    //$this->form_validation->set_rules('user_email','E-mel','trim|required|xss_clean');
    ...
    if ($this->form_validation->run())
    {
      ...
      $updatePengguna = $this->pentadbir_model->update_pengguna($myspata_userid,$dataDetail);
  


      if($updatePengguna)
      {
        $this->session->set_flashdata('flashComfirm', '<font color="blue" size="2">BERJAYA: Kata Laluan Pengguna Berjaya Dikemaskini</font><br>');
        redirect('pentadbir/pengurusan_pengguna');
      }
  ...
  function peranan_pengguna($id)
  {
    ...
    //$data['list_user'] = $this->pentadbir_model->get_listperanan();
    ...
    if($getListperanan = $this->pentadbir_model->get_listperanan())
    {
      ...
      $data['peranan_user'] = $this->pentadbir_model->get_peranan_user($id);
    }
    ...
    if ($this->form_validation->run())
    {
      $dataDetail = array('p' => $this->input->post('password'));
  ...
  function pengurusan_pengguna()  //edited by seri, 20102013
  {
    if($_POST)
        {
