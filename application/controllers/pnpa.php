  ...
  function arahan_penyediaan_pnpa_ppd()
  {
   ...
   if($this->input->post('hantar')=='hantar'){
      ...
      if($this->input->post('pla')!='' && $i==0)
        ...
        //$notifikasi_id =  $this->pnpa_model->get_noti_pelan($id);
        $path = 'pspao/senarai_pspao_akhir_ppd_2/'.$id.'/999';
...
function arahan_penyediaan_pnpa_ppd2()
  {
   ...
   if($this->input->post('hantar')=='hantar'){
      ...
      if($this->input->post('pla')!='' && $i==0)
        ...
        $notifikasi_id =  $this->pnpa_model->get_noti_pelan($id);
        $path = 'pspao/senarai_pspao_akhir_ppd_2/'.$id.'/'.$notifikasi_id;
 ...
 if ($_POST)
            {
             ...
             if($this->input->post('sumberluaran') == 'Simpan')
               ...
               else if ($this->input->post('hantar') == 'Simpan Deraf')
                 {
                   ...
                   $this->pnpa_model->insert_status_log($this->uri->segment(3),1,$this->uri->segment(4)); //1=xx,2=xxx,3=pembetulan,4=sah
        //echo 'ok';
                    ...
                    $data['link'] = 'pnpa/senarai_ppd_pnpa/'.$this->uri->segment(3);
                    $this->load->view('template/default', $data);
...
//desc : for summary_pp_pnpa view
  function model_struktur_pnpa_edit_ppd ()
  {
              ...
              if($this->form_validation->run())
            {
              ...
              redirect('pnpa/summary_pnpa/'.$this->uri->segment(3).'/'.$this->uri->segment(4),'refresh');
...
/*
  ...
  */
  ...
  //name : fatin
//date : 26/10/13
//desc : for summary_ppd_pnpa view
...
//tambah panel penilai dekat pelan
  ...
  //added : diana 13/11/13 : function copy borang
	function copyborang()
	{		
		$pnpaIDCopy = $this->uri->segment(4);
		//$typeofPlan = $this->uri->segment(5); //1-pnpa,2-ptra
		
		//if($typeofPlan==1)
		//{
			if($this->salinplan_model->checkrekodcopy($pnpaIDCopy,'pnpa')==1)
			{
				$chk = $this->salinplan_model->set_copy_rekod($pnpaIDCopy,'pnpa');
				
				if($chk)
				{
					redirect('pnpa/senarai_ppd_pnpa/'.$this->uri->segment(3),'refresh');
				} 
				else
				{
					$this->session->set_flashdata('flashError', '<strong><font color="red" size="3">AMARAN : Gagal menyimpan rekod.</font><br></strong><br>');
			...
			{
				$this->session->set_flashdata('flashError', '<strong><font color="red" size="3">AMARAN : Gagal menyimpan rekod.</font><br></strong><br>');
				redirect('pnpa/senarai_ppd_pnpa/'.$this->uri->segment(3).'/'.$this->uri->segment(4),'refresh');
			}
		//}
	}
	
	//page copy created of pnpa
	function kandungan_pnpa_copy ()
	{
		$node_id = '14';
		$menu_name = 'menu1';
		$menu_link = 'pnpa/kandungan_pnpa_copy';
		...
		if($_POST)
        {
			$this->form_validation->set_rules('edit','-','trim|required|xss_clean');
            $this->form_validation->set_rules('tahun','Tahun','trim|required|xss_clean');
            // $this->form_validation->set_rules('jabatan','Jabatan','trim|required|xss_clean');
            $this->form_validation->set_rules('premis','Premis','trim|required|xss_clean');
            $this->form_validation->set_rules('namapremis','Nama Premis','trim|required|xss_clean');
			...
			if($this->form_validation->run())
			{
				$pnpa_id = $this->salinplan_model->updateborang_copy('pnpa');
				redirect('pnpa/summary_pnpa/'.$this->uri->segment(3).'/'.$this->uri->segment(4),'refresh');
			}
		}
