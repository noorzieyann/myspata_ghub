...
echo form_open('pspao/senarai_pspao_akhir_ketuajab_2',$attributes); 
echo '
...
<?php for($i=2015;$i>2000;$i--){ ?>
                      <option value="<?php echo $i ?>"<?php if($i == 2013){echo 'selected="selected"';} ?>><?php echo $i ?></option>
<?php } ?>
					</select>
...
<?php // SENARAI BORANG -END- ?>
        ...
        <button class="btn btn-success" type="button" id="confirm"> Lulus </button>
...
<script>
    ...
    $('#nowucme17').wysihtml5();
      ...
      $("confirm").onclick = function () {
        reset();
        alertify.confirm("Adakah anda ingin meluluskan PSPA(O) Akhir?", function (e) {
          if (e) {
            alertify.success("PSPAO berjaya diluluskan");
			window.location.replace("<?php echo site_url('pspao/pspao_akhir') ?>");
          } else {
            alertify.error("PSPAO dibatalkan");
          }
        });
        return false;
      };
