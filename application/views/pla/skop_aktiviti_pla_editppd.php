            ...
            <div class="span12">
                <div class="widget">
                  ...
                  <table class="table table-condensed table-striped table-bordered table-hover no-margin">
                    ...
                    <?php $i=1; ?>
                    <?php 
					if($getSko = $this->skopaktiviti_model->get_lkpskop($this->uri->segment(4),'pla')):
                        ...
                        <tr>
							...
							<?php   
								$loop_aktvt_cnt=0;
								if($getSko = $this->skopaktiviti_model->get_lkpskopaktiviti($rr->skop_aktvt_id, $this->uri->segment(4),'pla')) :
                       ...
                       <?php $cnt=$this->skopaktiviti_model->get_lkpskopbutiran_count($rrr->skop_aktvt_id,'pla');
                          $loop_cnt=0;
						?>
                        
                         <?php if($getSko = $this->skopaktiviti_model->get_lkpskopbutiran($rrr->skop_aktvt_id,  $this->uri->segment(4),'pla')):
								foreach ($getSko as $rrq) :?>
								<td class="hidden-phone"> <?php $a=$rrq->skop_aktvt_tajuk; echo $a;?></td> 
                       
								<td class="hidden-phone"><?php $bajet_akt = $this->skopaktiviti_model->cal_bajet_skop($this->uri->segment(4),$rrq->skop_aktvt_id,'pla'); echo $bajet_akt; ?></td>
								<td class="hidden-phone"><span class="badge badge-success"><?php if($bajet_akt>0){ echo "Lengkap"; }else{ echo "Tidak Lengkap"; } ?></span></td>
								<td class="hidden-phone"><a href="<?php echo site_url()?>/pla/skop_aktiviti2_pla_editppd/<?php echo $this->uri->segment(3) ?>/<?php echo $this->uri->segment(4) ?>/<?php  echo $rrq->skop_aktvt_id; ?>">
...
<?php $plaID = $this->uri->segment(4); ?>
					<input type="text" class="input-large required" disabled="disabled"  placeholder="" value="<?php echo $this->skopaktiviti_model->cal_bajet($plaID,'pla'); ?>" readonly /></label>
					<!--end panel skop --> 
                     </div>                     
                  </div>  
                  <!--/.END-->                    
        ...
        <!--/.END panel 1-->
                ...
                <div class="widget-body" align="right">
                    <a href="<?php echo site_url('pla/summary_pla/'.$this->uri->segment(3).'/'.$this->uri->segment(4)) ?>"><button class="btn btn-danger input-top-margin" type="button">
                    Batal
                  </button></a>
                  <a href="<?php echo site_url('pla/treeview_pla_editppd/'.$this->uri->segment(3).'/'.$this->uri->segment(4).'/'.'edit') ?>"><button class="btn btn-primary input-top-margin" type="button">
                    Sebelum
                  </button></a>
  ...
  <!--/.END tab section-->
    ...
    <!--/.END tab navigation-->
<?php echo form_open('pla/skop_aktiviti_pla_editppd/'.$this->uri->segment(3).'/'.$this->uri->segment(4).'/'.$this->uri->segment(5)); ?>
