...
function get_pof()
  ...
  function count_data_in_tbl_pnpa_pata_f8_model(){

         $this->db->select('*');
         $this->db->where('pnpa_id', $this->uri->segment(4));
        // $this->db->where('utiliti_sumber_man_id',$this->uri->segment(4));
         $getdata = $this->db->get('tbl_pnpa_pata_f8_1a_modelstruktur');
...
function get_sumber_rancang($utiliti_sumber_man_id,$pnpa_id,$kat_sumber_man_id, $skop_aktvt_id)
	...
	function get_total_pejabat($pnpa_id,$skop_aktvt_id,$urus_pej_id,$kat_alat_pej_id)
    {
        $this->db->where('pnpa_id', $pnpa_id);
		$this->db->where('skop_aktvt_id', $skop_aktvt_id);
        ...
        $getUrusSya = $this->db->get('tbl_pnpa_pata_f8_1b1c_urus_pej');
        
        return $getUrusSya->result();
        
    }
	//added : diana 7/11/2013
	function get_total_peralatan($pnpa_id,$skop_aktvt_id,$alat_kerja_id,$kat_alat_kerja_id)
    {
        $this->db->where('pnpa_id', $pnpa_id);
		$this->db->where('skop_aktvt_id', $skop_aktvt_id);
        ...
        $getUrusSya = $this->db->get('tbl_pnpa_pata_f8_1b1c_alat_kerja');
...
*/
    ...
    */
    ...
    {
			$this->db->where("pnpa_pata_f8_1b1c_sumber_man_id", $idsumber);
			$this->db->delete("tbl_pnpa_pata_f8_1b1c_sumber_man");
	}
...
//desc : popup status
	...
	*/
	...
	function cal_man_cost($pnpa_id)
	{
		...
		$this->db->select('*');
		$this->db->where('pnpa_id', $pnpa_id);
		$this->db->where('kat_sumber_man_id', 3);
  ...
  function get_status_summary($pnpa_id)
  {
    ...
    $this->db->select('*');
    $this->db->from('tbl_pnpa_pata_f8_1a_modelstruktur');
    $this->db->join('tbl_pnpa_pata_f8_1a_pelan_kom','tbl_pnpa_pata_f8_1a_pelan_kom.pnpa_id= tbl_pnpa_pata_f8_1a_modelstruktur.pnpa_id');
    $this->db->join('tbl_pnpa_pata_f8_1a_panel_penilai', 'tbl_pnpa_pata_f8_1a_panel_penilai.pnpa_id = tbl_pnpa_pata_f8_1a_modelstruktur.pnpa_id');
    $this->db->where('tbl_pnpa_pata_f8_1a_modelstruktur.pnpa_id', $pnpa_id);
  ...
  function get_status_summary2($pnpa_id)
  {
    ...
    $this->db->select('*');
    $this->db->from('tbl_pnpa_pata_f8_1b_skop_pilihan');
    $this->db->join('tbl_pnpa_pata_f8_1b1c','tbl_pnpa_pata_f8_1b1c.pnpa_id= tbl_pnpa_pata_f8_1b_skop_pilihan.pnpa_id');
    $this->db->where('tbl_pnpa_pata_f8_1b_skop_pilihan.pnpa_id', $pnpa_id);
  ...
  function get_status_summary3($pnpa_id)
  {
    $this->db->select('*');
    $this->db->from('tbl_pnpa_pata_f8_1d');
    $this->db->where('pnpa_id', $pnpa_id);
    ...
    if ($query->num_rows() > 0)
    {
      ...
      if ($row->pnpa_id != NULL && $row->f8_1d_jenis_rekod != NULL && $row->f8_1d_lokasi != NULL && $row->tempoh != NULL)
      {
        return true;
      }
  ...
  function get_status_summary4($pnpa_id)
  {
    $this->db->select('*');
    $this->db->from('tbl_lampiran');
    $this->db->where('pnpa_id', $pnpa_id);
