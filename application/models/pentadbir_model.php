...
function get_namakem($idkem)
   ...
   function get_peranan()  //dapatkan list peranan pengguna
 {
   ...
   function get_peranan_user($id)  //dapatkan list peranan pengguna
 {
   $this->db->select('a.role_pengguna_id');
   $this->db->from('tbl_user_matrix b');
   $this->db->join('tbl_myspata_user_to_matrix a','a.role_pengguna_id = b.role_pengguna_id');
   $this->db->where('a.myspata_userid',$id);
   $this->db->group_by("b.role_pengguna_id", "ASC");
   $this->db->order_by("b.role_pengguna", "ASC");
   //$this->db->where('',$id);
   $query = $this->db->get();
   ...
   function get_modul()  //dapatkan list modul pengguna
 {
   ...
   function daftar_pengguna()
    ...
    function update_pengguna($myspata_userid,$dataDetail)
    {
        date_default_timezone_set('Asia/Kuala_Lumpur');
       $this->db->where('myspata_userid', $myspata_userid);
        $pengguna = $this->db->update('tbl_myspata_user', $dataDetail);
        if($kerja)
        {
            $dataLog = array('myspata_userid' => $this->session->userdata('user_id'),
                             'pswd' => 'kemaskini kata laluan pengguna',
